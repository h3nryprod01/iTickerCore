...
// NOTE: This endpoint is unofficial (iboard-api.ssi.com.vn). Validated against
// the UDF history shape only; the exact field names are based on public UDF
// conventions and fixture data. Live schema not verified — may drift.
// UDF shape: { "s":"ok"|"no_data"|"error", "t":[unix], "o":[], "h":[], "l":[], "c":[], "v":[] }
private struct SSIHistoryResponse: Decodable {
...
public struct SSIProvider: PriceProvider {
    ...
    public init(
        ...
        baseURL: URL = URL(string: "https://iboard-api.ssi.com.vn")!
    ) {
    ...
    private func fetchQuote(instrument: Instrument) async throws(ProviderError) -> Quote? {
        ...
        guard response.s == "ok" else {
            ...
            throw .unavailable("SSI returned status: \(response.s)")
        }
        ...
        // SSI iboard-api returns prices in THOUSANDS of VND (same convention as TCBS and VNDirect).
    ...
    private func fetchHistory(symbol: String, from: Int, to: Int) async throws(ProviderError) -> SSIHistoryResponse {
        var components = URLComponents(
            url: baseURL.appendingPathComponent("statistics/charts/history"),
